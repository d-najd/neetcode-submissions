class Solution {
   ...
   findWord(board: string[][], word: string, x: number, y: number, checked: number[][]) {
        //console.log(`CHECKING ${x} ${y}: ${checked}`)
        if (this.result.includes(word)) return
        if (board[y][x] !== word[checked.length]) return 
        if (checked.some(([xc, yc]) => xc === x && yc === y)) return
        
        //console.log(`PD ${x} ${y}: ${checked}`)
        ...
        this.findWord(board, word, Math.min(board.length, x+1), y, newChecked)
